...
function allowedTargetRoles({ invitedByPlatformRole, invitedByRole }) {
  ...
  switch (invitedByRole) {
    case 'VENDOR_OWNER':
      return new Set(['VENDOR_STAFF', 'VENDOR_RIDER'])
    default:
      return null
  }
}
...
export class VendorEmployeesService {
   ...
   *       as the `vendor_employees` insert and the `staff_created` audit row
   ...
   *     `requirePermission('vendor_employees.create')` — those guards filter
   ...
   *     `vendor_employees` + a fire-and-forget audit emit. The mutation is one
   *     statement, so a tx would only add a connection round-trip cost.
   *   - New-user shape: one tx wraps INSERT users → INSERT vendor_employees →
   ...
   * `vendor_employees` plus a fire-and-forget audit emit. The duplicate +
   ...
   * to the target shop via `vendor_employees`, and emits a `staff_created`
  ...
  async _createWithNewUser({ data, ctx, shopId, role, permissions }) {
    ...
    // violation.
    if (email && typeof this.repo.findUserByEmailCI === 'function') {
    ...
    // ── Atomic: INSERT users + INSERT vendor_employees + audit ────────────
      ...
      try {
        ...
        try {
          ...
          logger.error(
            ...
            'vendor_employees create ROLLBACK failed',
          )
        }
        throw err
      } finally {
  ...
  async update(
    ...
    // vendor_employees.is_active AND vendor_employees.deleted_at AND shop.is_active —
    ...
    try {
      ...
      try {
        ...
        logger.error(
          ...
          'vendor_employees deactivate ROLLBACK failed'
        )
      }
      throw err
    } finally {
   ...
   *   The route handler runs `requirePermission('vendor_employees.reset_password')`
    ...
    try {
      ...
      try {
        ...
        logger.error(
          ...
          'vendor_employees resetPassword ROLLBACK failed'
        )
      }
      throw err
    } finally {
