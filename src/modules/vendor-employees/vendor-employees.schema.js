...
/**
 * Valid shop staff roles (Requirement 2.1, R16.8). VENDOR_RIDER is a
 * vendor-owned delivery rider — distinct from the unrelated platform-wide
 * `RIDER` value on the users.role enum used by the independent gig-rider
 * system in src/modules/delivery/.
 */
export const VALID_ROLES = ['VENDOR_OWNER', 'VENDOR_STAFF', 'VENDOR_RIDER']
...
const newUserCreateSchema = z
  .object({
    ...
    // can use a plain `=` comparison. Optional overall: a VENDOR_RIDER logs
    // in via phone-OTP only and never uses the email+password admin login,
    // so email is meaningless for that role (enforced below).
    email: z
      .string()
  ...
  .superRefine((data, ctx) => {
    if (data.role === 'VENDOR_RIDER') {
      if (!data.phone) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'phone is required for a VENDOR_RIDER — it is their only login credential',
          path: ['phone'],
        })
      }
    } else if (!data.email) {
